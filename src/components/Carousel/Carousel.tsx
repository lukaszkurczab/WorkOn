      ...
      if (event.translationX > 0) {
        if (distance.value <= changeWidth && currentIndex.value > 0) {
          ...
          prevTranslateX.value =
            -64 - changeWidth + distance.value - changeWidth + (64 / changeWidth) * (distance.value - changeWidth);
          currentTranslateX.value =
            changeWidth - distance.value + changeWidth + (64 / changeWidth) * (distance.value - changeWidth);
          prevScale.value = Math.max(0.8 + 0.2 * (distance.value / adjustedTranslationX));
        }
      } else {
        ...
        <View style={styles.container}>
          <View style={styles.itemsWrapper}>
            {items.map((item, index) => {
              const animatedStyle = useAnimatedStyle(() => ({
                transform:
                  index === currentIndex.value
                    ? [{ scale: currentScale.value }, { translateX: currentTranslateX.value }]
              ...
              return (
                <Animated.View key={item.id} style={[styles.itemContainer, animatedStyle]}>
                  {item.component}
                </Animated.View>
              );
            })}
